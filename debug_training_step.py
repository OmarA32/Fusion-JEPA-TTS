...
def run_debug(lang="arabic", db="nawar_halabi", ckpt_path="training_logs/arabic/last.ckpt"):
    ...
    with torch.no_grad():
        ...
        # MAE Encoder & Decoder
        x_enc = model.forward_encoder(x, mask, class_embedding)
        z = model.forward_decoder(x_enc, mask, class_embedding)
    ...
    axes[1, 2].set_title('x1 (Real Ground Truth)')

    plt.tight_layout()
    os.makedirs('test_results', exist_ok=True)
    out_path = f"test_results/euler_step_debug_{lang}.png"
    plt.savefig(out_path)
    print(f"Saved visualization to {out_path}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Debug the Flow Matching training step.")
    ...
    run_debug(lang=args.lang, db=args.db, ckpt_path=args.ckpt)

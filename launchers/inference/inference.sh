...
if [ $# -eq 0 ]; then
    ...
    echo ""
    echo "  Examples:"
    echo "    bash launchers/inference/inference.sh --lang english --text \"Hello world from Fusion-JEPA\""
    echo "    bash launchers/inference/inference.sh --lang arabic --text \"مرحبا بكم\" --save-mel"
    echo "    bash launchers/inference/inference.sh --lang english --db ljspeech --index 108 --save-mel"
    echo "=============================================================================="
    read -p "Enter arguments (or press Enter for default Arabic synthesis): " -r user_args
    python pipelines/inference.py $user_args
else

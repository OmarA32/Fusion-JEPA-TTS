...
if [ $# -eq 0 ]; then
    ...
    echo ""
    echo "  Examples:"
    echo "    bash launchers/train.sh --lang arabic --db nawar_halabi --resume --checkpointnum 150"
    echo "    bash launchers/train.sh --lang english --db ljspeech --resume --checkpointnum 80"
    echo "=============================================================================="
    read -p "Enter arguments (or press Enter for default Arabic training): " -r user_args
    python train.py $user_args
else
    python train.py "$@"
fi

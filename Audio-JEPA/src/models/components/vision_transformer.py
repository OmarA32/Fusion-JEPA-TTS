...
try:
    ...
    import torch.nn as nn
    class MHA(nn.Module):
        def __init__(self, embed_dim, num_heads, dropout=0., qkv_proj_bias=True, use_flash_attn=False, **kwargs):
            super().__init__()
            self.mha = nn.MultiheadAttention(embed_dim, num_heads, dropout=dropout, bias=qkv_proj_bias, batch_first=True)
